    ...
    <PageSection>
      ...
      <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        {data.teachers.map((agent) => {
          ...
          return (
            <Card key={agent.id} as="article" className="paper-grain flex flex-col">
              <div className="flex items-start gap-4">
                <AvatarMark label={l(agent.name, locale)} tone={agent.avatarTone} />
                <div>
                  <h2 className="text-xl font-semibold tracking-tight text-[var(--foreground)]">{l(agent.name, locale)}</h2>
                  <p className="mt-1 text-sm font-medium text-[var(--accent-soft)]">{l(agent.method, locale)}</p>
                </div>
              </div>
              <p className="mt-4 text-sm leading-7 text-[var(--text-muted)]">{l(agent.description, locale)}</p>
              ...
              <div className="mt-5">
                <h3 className="text-sm font-semibold text-[var(--foreground)]">{dictionary.education.sampleLessons}</h3>
                <div className="mt-3 space-y-2">
                  {agent.lessons.slice(0, 3).map((lesson) => (
                    <div key={lesson.id} className="flex items-center justify-between gap-3 rounded-xl border border-[var(--border-soft)] bg-[var(--surface-warm)]/70 px-3 py-2">
                      <span className="text-sm text-[var(--text-muted)]">{l(lesson.title, locale)}</span>
              ...
              <div className="mt-5">
                <h3 className="text-sm font-semibold text-[var(--foreground)]">{dictionary.education.sampleExercises}</h3>
